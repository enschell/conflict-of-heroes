/**
 * New Mission
 * Generated by the Mission Editor — a self-contained Mission definition.
 *
 * To make this Mission playable:
 *  1. Drop this file into src/data/missions/.
 *  2. Import `NEW_MISSION_MISSION` and add a button for it in src/ui/SetupScreen.tsx.
 *  3. Optionally register it in src/data/missions/catalog.ts for online play.
 */
import type { MissionDef } from '../../engine/types';

export const NEW_MISSION_MISSION: MissionDef = {
  id: 'new-mission',
  name: 'New Mission',
  roundsTotal: 5,
  seed: 1783903034259,
  caps: {
    A: 6,
    B: 6,
  },
  nations: {
    A: [
      'germans',
    ],
    B: [
      'soviets',
    ],
  },
  firstInitiative: 'A',
  hexes: [
    {
      id: '0,0',
      terrain: 'open',
      boardNumber: 1,
      mapNumber: 1,
    },
    {
      id: '0,1',
      terrain: 'open',
      label: 'A01',
      mapNumber: 1,
    },
    {
      id: '0,2',
      terrain: 'open',
      label: 'A02',
      mapNumber: 1,
    },
    {
      id: '0,3',
      terrain: 'open',
      label: 'A03',
      mapNumber: 1,
    },
    {
      id: '0,4',
      terrain: 'open',
      label: 'A04',
      mapNumber: 1,
    },
    {
      id: '0,5',
      terrain: 'open',
      label: 'A05',
      mapNumber: 1,
    },
    {
      id: '0,6',
      terrain: 'open',
      label: 'A06',
      mapNumber: 1,
      road: true,
    },
    {
      id: '0,7',
      terrain: 'open',
      label: 'A07',
      mapNumber: 1,
    },
    {
      id: '0,8',
      terrain: 'open',
      label: 'A08',
      mapNumber: 1,
    },
    {
      id: '0,9',
      terrain: 'open',
      label: 'A09',
      mapNumber: 1,
    },
    {
      id: '0,10',
      terrain: 'open',
      label: 'A10',
      mapNumber: 1,
    },
    {
      id: '0,11',
      terrain: 'open',
      label: 'A11',
      mapNumber: 1,
    },
    {
      id: '0,12',
      terrain: 'open',
      label: 'A12',
      mapNumber: 1,
    },
    {
      id: '1,0',
      terrain: 'open',
      label: 'B01',
      mapNumber: 1,
    },
    {
      id: '1,1',
      terrain: 'open',
      label: 'B02',
      mapNumber: 1,
    },
    {
      id: '1,2',
      terrain: 'open',
      label: 'B03',
      mapNumber: 1,
    },
    {
      id: '1,3',
      terrain: 'woodsLight',
      label: 'B04',
      mapNumber: 1,
    },
    {
      id: '1,4',
      terrain: 'woodsLight',
      label: 'B05',
      mapNumber: 1,
    },
    {
      id: '1,5',
      terrain: 'open',
      label: 'B06',
      mapNumber: 1,
      road: true,
    },
    {
      id: '1,6',
      terrain: 'open',
      label: 'B07',
      mapNumber: 1,
    },
    {
      id: '1,7',
      terrain: 'open',
      label: 'B08',
      mapNumber: 1,
    },
    {
      id: '1,8',
      terrain: 'open',
      label: 'B09',
      mapNumber: 1,
    },
    {
      id: '1,9',
      terrain: 'open',
      label: 'B10',
      mapNumber: 1,
    },
    {
      id: '1,10',
      terrain: 'open',
      label: 'B11',
      mapNumber: 1,
    },
    {
      id: '1,11',
      terrain: 'open',
      label: 'B12',
      mapNumber: 1,
    },
    {
      id: '2,-1',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '2,0',
      terrain: 'open',
      label: 'C01',
      mapNumber: 1,
    },
    {
      id: '2,1',
      terrain: 'open',
      label: 'C02',
      mapNumber: 1,
    },
    {
      id: '2,2',
      terrain: 'open',
      label: 'C03',
      mapNumber: 1,
    },
    {
      id: '2,3',
      terrain: 'woodsLight',
      label: 'C04',
      mapNumber: 1,
    },
    {
      id: '2,4',
      terrain: 'open',
      label: 'C05',
      mapNumber: 1,
      road: true,
    },
    {
      id: '2,5',
      terrain: 'open',
      label: 'C06',
      mapNumber: 1,
    },
    {
      id: '2,6',
      terrain: 'open',
      label: 'C07',
      mapNumber: 1,
    },
    {
      id: '2,7',
      terrain: 'open',
      label: 'C08',
      mapNumber: 1,
    },
    {
      id: '2,8',
      terrain: 'woodsLight',
      label: 'C09',
      mapNumber: 1,
    },
    {
      id: '2,9',
      terrain: 'open',
      label: 'C10',
      mapNumber: 1,
    },
    {
      id: '2,10',
      terrain: 'open',
      label: 'C11',
      mapNumber: 1,
    },
    {
      id: '2,11',
      terrain: 'open',
      label: 'C12',
      mapNumber: 1,
    },
    {
      id: '3,-1',
      terrain: 'open',
      label: 'D01',
      mapNumber: 1,
    },
    {
      id: '3,0',
      terrain: 'woodsLight',
      label: 'D02',
      mapNumber: 1,
    },
    {
      id: '3,1',
      terrain: 'open',
      label: 'D03',
      mapNumber: 1,
    },
    {
      id: '3,2',
      terrain: 'open',
      label: 'D04',
      mapNumber: 1,
    },
    {
      id: '3,3',
      terrain: 'open',
      label: 'D05',
      mapNumber: 1,
      road: true,
    },
    {
      id: '3,4',
      terrain: 'open',
      label: 'D06',
      mapNumber: 1,
    },
    {
      id: '3,5',
      terrain: 'open',
      label: 'D07',
      mapNumber: 1,
    },
    {
      id: '3,6',
      terrain: 'open',
      label: 'D08',
      mapNumber: 1,
    },
    {
      id: '3,7',
      terrain: 'open',
      label: 'D09',
      mapNumber: 1,
    },
    {
      id: '3,8',
      terrain: 'open',
      label: 'D10',
      mapNumber: 1,
    },
    {
      id: '3,9',
      terrain: 'open',
      label: 'D11',
      mapNumber: 1,
    },
    {
      id: '3,10',
      terrain: 'open',
      label: 'D12',
      mapNumber: 1,
    },
    {
      id: '4,-2',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '4,-1',
      terrain: 'woodsHeavy',
      label: 'E01',
      mapNumber: 1,
    },
    {
      id: '4,0',
      terrain: 'open',
      label: 'E02',
      mapNumber: 1,
    },
    {
      id: '4,1',
      terrain: 'open',
      label: 'E03',
      mapNumber: 1,
    },
    {
      id: '4,2',
      terrain: 'open',
      label: 'E04',
      mapNumber: 1,
    },
    {
      id: '4,3',
      terrain: 'open',
      label: 'E05',
      mapNumber: 1,
      road: true,
    },
    {
      id: '4,4',
      terrain: 'woodsLight',
      label: 'E06',
      mapNumber: 1,
    },
    {
      id: '4,5',
      terrain: 'woodsHeavy',
      label: 'E07',
      mapNumber: 1,
    },
    {
      id: '4,6',
      terrain: 'open',
      label: 'E08',
      mapNumber: 1,
    },
    {
      id: '4,7',
      terrain: 'woodsLight',
      label: 'E09',
      mapNumber: 1,
    },
    {
      id: '4,8',
      terrain: 'woodsLight',
      label: 'E10',
      mapNumber: 1,
    },
    {
      id: '4,9',
      terrain: 'open',
      label: 'E11',
      mapNumber: 1,
    },
    {
      id: '4,10',
      terrain: 'open',
      label: 'E12',
      mapNumber: 1,
    },
    {
      id: '5,-2',
      terrain: 'open',
      label: 'F01',
      mapNumber: 1,
    },
    {
      id: '5,-1',
      terrain: 'open',
      label: 'F02',
      mapNumber: 1,
    },
    {
      id: '5,0',
      terrain: 'open',
      label: 'F03',
      mapNumber: 1,
    },
    {
      id: '5,1',
      terrain: 'open',
      label: 'F04',
      mapNumber: 1,
    },
    {
      id: '5,2',
      terrain: 'open',
      label: 'F05',
      mapNumber: 1,
    },
    {
      id: '5,3',
      terrain: 'open',
      label: 'F06',
      mapNumber: 1,
      road: true,
    },
    {
      id: '5,4',
      terrain: 'woodsHeavy',
      label: 'F07',
      mapNumber: 1,
    },
    {
      id: '5,5',
      terrain: 'open',
      label: 'F08',
      mapNumber: 1,
    },
    {
      id: '5,6',
      terrain: 'woodsLight',
      label: 'F09',
      mapNumber: 1,
    },
    {
      id: '5,7',
      terrain: 'open',
      label: 'F10',
      mapNumber: 1,
      road: false,
    },
    {
      id: '5,8',
      terrain: 'open',
      label: 'F11',
      mapNumber: 1,
    },
    {
      id: '5,9',
      terrain: 'open',
      label: 'F12',
      mapNumber: 1,
    },
    {
      id: '6,-3',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '6,-2',
      terrain: 'open',
      label: 'G01',
      mapNumber: 1,
    },
    {
      id: '6,-1',
      terrain: 'open',
      label: 'G02',
      mapNumber: 1,
    },
    {
      id: '6,0',
      terrain: 'open',
      label: 'G03',
      mapNumber: 1,
    },
    {
      id: '6,1',
      terrain: 'open',
      label: 'G04',
      mapNumber: 1,
    },
    {
      id: '6,2',
      terrain: 'open',
      label: 'G05',
      mapNumber: 1,
      road: true,
    },
    {
      id: '6,3',
      terrain: 'woodsHeavy',
      label: 'G06',
      mapNumber: 1,
    },
    {
      id: '6,4',
      terrain: 'woodsHeavy',
      label: 'G07',
      mapNumber: 1,
    },
    {
      id: '6,5',
      terrain: 'open',
      label: 'G08',
      mapNumber: 1,
      road: false,
    },
    {
      id: '6,6',
      terrain: 'open',
      label: 'G09',
      mapNumber: 1,
      road: true,
    },
    {
      id: '6,7',
      terrain: 'open',
      label: 'G10',
      mapNumber: 1,
      road: true,
    },
    {
      id: '6,8',
      terrain: 'open',
      label: 'G11',
      mapNumber: 1,
    },
    {
      id: '6,9',
      terrain: 'open',
      label: 'G12',
      mapNumber: 1,
    },
    {
      id: '7,-3',
      terrain: 'open',
      label: 'H01',
      mapNumber: 1,
    },
    {
      id: '7,-2',
      terrain: 'open',
      label: 'H02',
      mapNumber: 1,
    },
    {
      id: '7,-1',
      terrain: 'open',
      label: 'H03',
      mapNumber: 1,
    },
    {
      id: '7,0',
      terrain: 'open',
      label: 'H04',
      mapNumber: 1,
    },
    {
      id: '7,1',
      terrain: 'open',
      label: 'H05',
      mapNumber: 1,
    },
    {
      id: '7,2',
      terrain: 'open',
      label: 'H06',
      mapNumber: 1,
      road: true,
    },
    {
      id: '7,3',
      terrain: 'open',
      label: 'H07',
      mapNumber: 1,
      road: true,
    },
    {
      id: '7,4',
      terrain: 'open',
      label: 'H08',
      mapNumber: 1,
      road: true,
    },
    {
      id: '7,5',
      terrain: 'open',
      label: 'H09',
      mapNumber: 1,
      road: true,
    },
    {
      id: '7,6',
      terrain: 'open',
      label: 'H10',
      mapNumber: 1,
    },
    {
      id: '7,7',
      terrain: 'open',
      label: 'H11',
      mapNumber: 1,
      road: true,
    },
    {
      id: '7,8',
      terrain: 'open',
      label: 'H12',
      mapNumber: 1,
    },
    {
      id: '8,-4',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '8,-3',
      terrain: 'open',
      label: 'I01',
      mapNumber: 1,
    },
    {
      id: '8,-2',
      terrain: 'open',
      label: 'I02',
      mapNumber: 1,
    },
    {
      id: '8,-1',
      terrain: 'open',
      label: 'I03',
      mapNumber: 1,
    },
    {
      id: '8,0',
      terrain: 'open',
      label: 'I04',
      mapNumber: 1,
    },
    {
      id: '8,1',
      terrain: 'open',
      label: 'I05',
      mapNumber: 1,
      road: false,
    },
    {
      id: '8,2',
      terrain: 'open',
      label: 'I06',
      mapNumber: 1,
      road: true,
    },
    {
      id: '8,3',
      terrain: 'open',
      label: 'I07',
      mapNumber: 1,
      road: true,
    },
    {
      id: '8,4',
      terrain: 'open',
      label: 'I08',
      mapNumber: 1,
    },
    {
      id: '8,5',
      terrain: 'open',
      label: 'I09',
      mapNumber: 1,
    },
    {
      id: '8,6',
      terrain: 'woodsLight',
      label: 'I10',
      mapNumber: 1,
    },
    {
      id: '8,7',
      terrain: 'open',
      label: 'I11',
      mapNumber: 1,
      road: true,
    },
    {
      id: '8,8',
      terrain: 'open',
      label: 'I12',
      mapNumber: 1,
    },
    {
      id: '9,-4',
      terrain: 'open',
      label: 'J01',
      mapNumber: 1,
      road: true,
    },
    {
      id: '9,-3',
      terrain: 'open',
      label: 'J02',
      mapNumber: 1,
    },
    {
      id: '9,-2',
      terrain: 'open',
      label: 'J03',
      mapNumber: 1,
    },
    {
      id: '9,-1',
      terrain: 'open',
      label: 'J04',
      mapNumber: 1,
    },
    {
      id: '9,0',
      terrain: 'open',
      label: 'J05',
      mapNumber: 1,
      road: false,
    },
    {
      id: '9,1',
      terrain: 'woodsLight',
      label: 'J06',
      mapNumber: 1,
    },
    {
      id: '9,2',
      terrain: 'open',
      label: 'J07',
      mapNumber: 1,
    },
    {
      id: '9,3',
      terrain: 'open',
      label: 'J08',
      mapNumber: 1,
    },
    {
      id: '9,4',
      terrain: 'open',
      label: 'J09',
      mapNumber: 1,
    },
    {
      id: '9,5',
      terrain: 'open',
      label: 'J10',
      mapNumber: 1,
    },
    {
      id: '9,6',
      terrain: 'open',
      label: 'J11',
      mapNumber: 1,
    },
    {
      id: '9,7',
      terrain: 'open',
      label: 'J12',
      mapNumber: 1,
      road: true,
    },
    {
      id: '10,-5',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '10,-4',
      terrain: 'open',
      label: 'K01',
      mapNumber: 1,
      road: true,
    },
    {
      id: '10,-3',
      terrain: 'woodsLight',
      label: 'K02',
      mapNumber: 1,
    },
    {
      id: '10,-2',
      terrain: 'woodsHeavy',
      label: 'K03',
      mapNumber: 1,
    },
    {
      id: '10,-1',
      terrain: 'open',
      label: 'K04',
      mapNumber: 1,
      road: false,
    },
    {
      id: '10,0',
      terrain: 'woodsLight',
      label: 'K05',
      mapNumber: 1,
    },
    {
      id: '10,1',
      terrain: 'open',
      label: 'K06',
      mapNumber: 1,
    },
    {
      id: '10,2',
      terrain: 'woodsLight',
      label: 'K07',
      mapNumber: 1,
    },
    {
      id: '10,3',
      terrain: 'open',
      label: 'K08',
      mapNumber: 1,
    },
    {
      id: '10,4',
      terrain: 'open',
      label: 'K09',
      mapNumber: 1,
    },
    {
      id: '10,5',
      terrain: 'open',
      label: 'K10',
      mapNumber: 1,
    },
    {
      id: '10,6',
      terrain: 'open',
      label: 'K11',
      mapNumber: 1,
    },
    {
      id: '10,7',
      terrain: 'open',
      label: 'K12',
      mapNumber: 1,
    },
    {
      id: '11,-5',
      terrain: 'open',
      label: 'L01',
      mapNumber: 1,
      road: true,
    },
    {
      id: '11,-4',
      terrain: 'woodsLight',
      label: 'L02',
      mapNumber: 1,
    },
    {
      id: '11,-3',
      terrain: 'woodsHeavy',
      label: 'L03',
      mapNumber: 1,
    },
    {
      id: '11,-2',
      terrain: 'woodsHeavy',
      label: 'L04',
      mapNumber: 1,
    },
    {
      id: '11,-1',
      terrain: 'woodsHeavy',
      label: 'L05',
      mapNumber: 1,
    },
    {
      id: '11,0',
      terrain: 'open',
      label: 'L06',
      mapNumber: 1,
    },
    {
      id: '11,1',
      terrain: 'open',
      label: 'L07',
      mapNumber: 1,
    },
    {
      id: '11,2',
      terrain: 'woodsHeavy',
      label: 'L08',
      mapNumber: 1,
    },
    {
      id: '11,3',
      terrain: 'open',
      label: 'L09',
      mapNumber: 1,
    },
    {
      id: '11,4',
      terrain: 'open',
      label: 'L10',
      mapNumber: 1,
    },
    {
      id: '11,5',
      terrain: 'open',
      label: 'L11',
      mapNumber: 1,
    },
    {
      id: '11,6',
      terrain: 'open',
      label: 'L12',
      mapNumber: 1,
    },
    {
      id: '12,-6',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '12,-5',
      terrain: 'open',
      label: 'M01',
      mapNumber: 1,
      road: true,
    },
    {
      id: '12,-4',
      terrain: 'open',
      label: 'M02',
      mapNumber: 1,
      road: true,
    },
    {
      id: '12,-3',
      terrain: 'open',
      label: 'M03',
      mapNumber: 1,
      road: true,
    },
    {
      id: '12,-2',
      terrain: 'woodsHeavy',
      label: 'M04',
      mapNumber: 1,
    },
    {
      id: '12,-1',
      terrain: 'open',
      label: 'M05',
      mapNumber: 1,
    },
    {
      id: '12,0',
      terrain: 'open',
      label: 'M06',
      mapNumber: 1,
    },
    {
      id: '12,1',
      terrain: 'woodsHeavy',
      label: 'M07',
      mapNumber: 1,
    },
    {
      id: '12,2',
      terrain: 'woodsHeavy',
      label: 'M08',
      mapNumber: 1,
    },
    {
      id: '12,3',
      terrain: 'open',
      label: 'M09',
      mapNumber: 1,
    },
    {
      id: '12,4',
      terrain: 'open',
      label: 'M10',
      mapNumber: 1,
    },
    {
      id: '12,5',
      terrain: 'open',
      label: 'M11',
      mapNumber: 1,
    },
    {
      id: '12,6',
      terrain: 'open',
      label: 'M12',
      mapNumber: 1,
    },
    {
      id: '13,-6',
      terrain: 'woodsHeavy',
      label: 'N01',
      mapNumber: 1,
    },
    {
      id: '13,-5',
      terrain: 'open',
      label: 'N02',
      mapNumber: 1,
      road: true,
    },
    {
      id: '13,-4',
      terrain: 'woodsLight',
      label: 'N03',
      mapNumber: 1,
    },
    {
      id: '13,-3',
      terrain: 'open',
      label: 'N04',
      mapNumber: 1,
    },
    {
      id: '13,-2',
      terrain: 'open',
      label: 'N05',
      mapNumber: 1,
    },
    {
      id: '13,-1',
      terrain: 'open',
      label: 'N06',
      mapNumber: 1,
    },
    {
      id: '13,0',
      terrain: 'woodsHeavy',
      label: 'N07',
      mapNumber: 1,
    },
    {
      id: '13,1',
      terrain: 'woodsHeavy',
      label: 'N08',
      mapNumber: 1,
    },
    {
      id: '13,2',
      terrain: 'woodsLight',
      label: 'N09',
      mapNumber: 1,
    },
    {
      id: '13,3',
      terrain: 'woodsLight',
      label: 'N10',
      mapNumber: 1,
    },
    {
      id: '13,4',
      terrain: 'open',
      label: 'N11',
      mapNumber: 1,
    },
    {
      id: '13,5',
      terrain: 'open',
      label: 'N12',
      mapNumber: 1,
    },
    {
      id: '14,-7',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '14,-6',
      terrain: 'open',
      label: 'O01',
      mapNumber: 1,
    },
    {
      id: '14,-5',
      terrain: 'open',
      label: 'O02',
      mapNumber: 1,
      road: true,
    },
    {
      id: '14,-4',
      terrain: 'open',
      label: 'O03',
      mapNumber: 1,
      road: true,
    },
    {
      id: '14,-3',
      terrain: 'open',
      label: 'O04',
      mapNumber: 1,
    },
    {
      id: '14,-2',
      terrain: 'open',
      label: 'O05',
      mapNumber: 1,
    },
    {
      id: '14,-1',
      terrain: 'open',
      label: 'O06',
      mapNumber: 1,
    },
    {
      id: '14,0',
      terrain: 'woodsLight',
      label: 'O07',
      mapNumber: 1,
    },
    {
      id: '14,1',
      terrain: 'open',
      label: 'O08',
      mapNumber: 1,
    },
    {
      id: '14,2',
      terrain: 'open',
      label: 'O09',
      mapNumber: 1,
    },
    {
      id: '14,3',
      terrain: 'open',
      label: 'O10',
      mapNumber: 1,
    },
    {
      id: '14,4',
      terrain: 'open',
      label: 'O11',
      mapNumber: 1,
    },
    {
      id: '14,5',
      terrain: 'open',
      label: 'O12',
      mapNumber: 1,
    },
    {
      id: '15,-7',
      terrain: 'open',
      label: 'P01',
      mapNumber: 1,
    },
    {
      id: '15,-6',
      terrain: 'open',
      label: 'P02',
      mapNumber: 1,
    },
    {
      id: '15,-5',
      terrain: 'woodsLight',
      label: 'P03',
      mapNumber: 1,
    },
    {
      id: '15,-4',
      terrain: 'open',
      label: 'P04',
      mapNumber: 1,
      road: true,
    },
    {
      id: '15,-3',
      terrain: 'woodsLight',
      label: 'P05',
      mapNumber: 1,
    },
    {
      id: '15,-2',
      terrain: 'woodsLight',
      label: 'P06',
      mapNumber: 1,
    },
    {
      id: '15,-1',
      terrain: 'open',
      label: 'P07',
      mapNumber: 1,
    },
    {
      id: '15,0',
      terrain: 'woodsLight',
      label: 'P08',
      mapNumber: 1,
    },
    {
      id: '15,1',
      terrain: 'open',
      label: 'P09',
      mapNumber: 1,
    },
    {
      id: '15,2',
      terrain: 'open',
      label: 'P10',
      mapNumber: 1,
    },
    {
      id: '15,3',
      terrain: 'open',
      label: 'P11',
      mapNumber: 1,
    },
    {
      id: '15,4',
      terrain: 'open',
      label: 'P12',
      mapNumber: 1,
    },
    {
      id: '16,-8',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '16,-7',
      terrain: 'open',
      label: 'Q01',
      mapNumber: 1,
    },
    {
      id: '16,-6',
      terrain: 'woodsLight',
      label: 'Q02',
      mapNumber: 1,
    },
    {
      id: '16,-5',
      terrain: 'woodsLight',
      label: 'Q03',
      mapNumber: 1,
    },
    {
      id: '16,-4',
      terrain: 'open',
      label: 'Q04',
      mapNumber: 1,
      road: true,
    },
    {
      id: '16,-3',
      terrain: 'open',
      label: 'Q05',
      mapNumber: 1,
      road: false,
    },
    {
      id: '16,-2',
      terrain: 'open',
      label: 'Q06',
      mapNumber: 1,
      road: false,
    },
    {
      id: '16,-1',
      terrain: 'woodsLight',
      label: 'Q07',
      mapNumber: 1,
    },
    {
      id: '16,0',
      terrain: 'open',
      label: 'Q08',
      mapNumber: 1,
    },
    {
      id: '16,1',
      terrain: 'open',
      label: 'Q09',
      mapNumber: 1,
    },
    {
      id: '16,2',
      terrain: 'woodsLight',
      label: 'Q10',
      mapNumber: 1,
    },
    {
      id: '16,3',
      terrain: 'open',
      label: 'Q11',
      mapNumber: 1,
    },
    {
      id: '16,4',
      terrain: 'open',
      label: 'Q12',
      mapNumber: 1,
    },
    {
      id: '17,-8',
      terrain: 'open',
      label: 'R01',
      mapNumber: 1,
    },
    {
      id: '17,-7',
      terrain: 'open',
      label: 'R02',
      mapNumber: 1,
    },
    {
      id: '17,-6',
      terrain: 'open',
      label: 'R03',
      mapNumber: 1,
    },
    {
      id: '17,-5',
      terrain: 'open',
      label: 'R04',
      mapNumber: 1,
    },
    {
      id: '17,-4',
      terrain: 'open',
      label: 'R05',
      mapNumber: 1,
      road: true,
    },
    {
      id: '17,-3',
      terrain: 'open',
      label: 'R06',
      mapNumber: 1,
      road: true,
    },
    {
      id: '17,-2',
      terrain: 'open',
      label: 'R07',
      mapNumber: 1,
      road: true,
    },
    {
      id: '17,-1',
      terrain: 'open',
      label: 'R08',
      mapNumber: 1,
    },
    {
      id: '17,0',
      terrain: 'open',
      label: 'R09',
      mapNumber: 1,
    },
    {
      id: '17,1',
      terrain: 'open',
      label: 'R10',
      mapNumber: 1,
    },
    {
      id: '17,2',
      terrain: 'open',
      label: 'R11',
      mapNumber: 1,
    },
    {
      id: '17,3',
      terrain: 'open',
      label: 'R12',
      mapNumber: 1,
    },
    {
      id: '18,-9',
      terrain: 'open',
      mapNumber: 1,
    },
    {
      id: '18,-8',
      terrain: 'open',
      label: 'S01',
      mapNumber: 1,
    },
    {
      id: '18,-7',
      terrain: 'open',
      label: 'S02',
      mapNumber: 1,
    },
    {
      id: '18,-6',
      terrain: 'open',
      label: 'S03',
      mapNumber: 1,
    },
    {
      id: '18,-5',
      terrain: 'open',
      label: 'S04',
      mapNumber: 1,
    },
    {
      id: '18,-4',
      terrain: 'open',
      label: 'S05',
      mapNumber: 1,
    },
    {
      id: '18,-3',
      terrain: 'open',
      label: 'S06',
      mapNumber: 1,
      road: true,
    },
    {
      id: '18,-2',
      terrain: 'open',
      label: 'S07',
      mapNumber: 1,
    },
    {
      id: '18,-1',
      terrain: 'open',
      label: 'S08',
      mapNumber: 1,
    },
    {
      id: '18,0',
      terrain: 'open',
      label: 'S09',
      mapNumber: 1,
    },
    {
      id: '18,1',
      terrain: 'open',
      label: 'S10',
      mapNumber: 1,
    },
    {
      id: '18,2',
      terrain: 'open',
      label: 'S11',
      mapNumber: 1,
    },
    {
      id: '18,3',
      terrain: 'open',
      label: 'S12',
      mapNumber: 1,
    },
  ],
  units: [],
  templates: [],
  victoryHexes: [],
  mapOverlays: {
    '1': '/assets/maps/map-1-mission-1-partisans.png',
  },
};
