/**
 * Time-boxed self-play harness — eighth round, follow-up to
 * scripts/selfplay1000tactics.ts (1000 games, the 4 Category profiles) and
 * scripts/selfplay30min.ts (30-minute time-boxed run, same profiles). Same
 * independent-rule-oracle technique as scripts/conformance.ts throughout;
 * same tactics-sheet-derived profiles (see selfplay1000tactics.ts's own
 * header for the full row-by-row mapping — unchanged here) PLUS one new
 * addition per the user's explicit ask this round:
 *
 * "don't just stick to one profile per game per side... decide when and
 * where to change tactics... use a combination of profiles" — a new
 * `dynamic` style (see chooseProfile()'s own header comment) that re-picks
 * which of the 5 concrete profiles governs ITS OWN side fresh on every
 * single Action, based on the live game state (Round number, this side's
 * own VP lead/deficit, its surviving-Unit ratio vs. the enemy, and who
 * currently controls the Victory Hex) — not locked to one profile for the
 * whole game, and not hardcoded to favor either side.
 *
 * Also fixes a scheduling bug the 30-minute round's own report flagged: the
 * pair-rotation cursor is now decoupled from the control-game counter (see
 * the schedule-building comment below for the full explanation), so every
 * pairing — including "X vs Balanced" — gets equal representation this time.
 *
 * Runs for 1 hour of real wall-clock time (Date.now()-based, same mechanism
 * as the 30-minute round) rather than a fixed game count.
 *
 *   npx tsx scripts/selfplay1hour.ts
 */
import {
  reduce,
  legalActions,
...
type Style = 'balanced' | 'ap_econ' | 'defense' | 'combat_group' | 'vp_focus' | 'random' | 'dynamic';
const STYLES: Style[] = ['balanced', 'ap_econ', 'defense', 'combat_group', 'vp_focus', 'random', 'dynamic'];
/** The 5 concrete profiles 'dynamic' is allowed to switch between — never
 *  resolves to itself or to 'random'. */
const SWITCHABLE_PROFILES: Style[] = ['balanced', 'ap_econ', 'defense', 'combat_group', 'vp_focus'];
...
const PROFILE_WEIGHTS: Record<Style, Weights> = {
  ...
  dynamic: { cover: 0, los: 0, hasty: 0, apCost: 0, elevation: 0 }, // never read directly — see chooseProfile
};
...
/**
 * The "dynamic" meta-controller (user request: "don't just stick to one
 * profile... decide when and where to change tactics... depending on the
 * changing game landscape"). Re-evaluated fresh every single Action this
 * side takes — not locked in for the whole game — using only information
 * available to a real player at that moment: the current Round, the no-tie
 * VP marker's sign from this side's own perspective, this side's own
 * surviving-Unit ratio vs. the enemy's, and who currently controls the
 * Mission's Victory Hex. Symmetric: the same rule applies whichever side
 * calls it, using each side's OWN relative state, not a hardcoded side bias
 * (unlike the old Hasty-Defense-favors-Soviets rule from an earlier round).
 *
 * Rationale for each branch, in priority order:
 *   1. Badly outnumbered (< 55% of the enemy's surviving Unit count) →
 *      Defense, regardless of anything else — survival first.
 *   2. Rounds 1-2 (opening) → AP Economy if already holding the objective
 *      (consolidate cheaply), else Victory Points (establish position early
 *      — row #36 Hold Objectives Early's own logic).
 *   3. Late Rounds (last 2) while ahead on VP → protect the lead: Defense if
 *      already holding the objective, else Combat & Group Actions to keep
 *      contesting it (a pure Victory-Points lock-down only makes sense once
 *      the objective is actually secured).
 *   4. Behind on VP → go looking for opportunities: Victory Points if this
 *      side already holds the objective (press the one advantage it has),
 *      else Combat & Group Actions (this series' best-replicated Soviet
 *      performer, rows #9/#29/#32) to force engagements instead of playing
 *      passively from behind.
 *   5. Otherwise (roughly even, mid-game) → Balanced.
 */
function chooseProfile(state: GameState, side: SideId): Style {
  const round = state.round;
  const totalRounds = state.roundsTotal;
  const myLead = side === 'A' ? state.vpMarker : -state.vpMarker; // >0 = I lead, <0 = I trail
  const myUnits = Object.values(state.units).filter((u) => u.side === side).length;
  const enemyUnits = Object.values(state.units).filter((u) => u.side !== side).length;
  const materialRatio = enemyUnits > 0 ? myUnits / enemyUnits : 2;
  const iControlObjective = state.hexes[OBJECTIVE_HEX]?.features.control === side;

  if (materialRatio < 0.55) return 'defense';
  if (round <= 2) return iControlObjective ? 'ap_econ' : 'vp_focus';
  if (round >= totalRounds - 1 && myLead > 0) return iControlObjective ? 'defense' : 'combat_group';
  if (myLead < 0) return iControlObjective ? 'vp_focus' : 'combat_group';
  return 'balanced';
}
...
function moveScoreFor(
  ...
  const extras = cover + los + apEff + elev;
...
function chooseAction(state: GameState, styleOf: Record<SideId, Style>, rand: () => number): Action {
  ...
  const rawStyle = styleOf[side];
  // 'dynamic' re-resolves to a concrete profile fresh every single Action —
  // see chooseProfile()'s own header comment for the full decision tree.
  const style = rawStyle === 'dynamic' ? chooseProfile(state, side) : rawStyle;
  ...
  // so there's never a reason for any profile to decline it).
...
function playGame(seed: number, styleA: Style, styleB: Style): GameResult {
  ...
  while (state.phase === 'playing' && steps < 10000) {
    const pre = state;
    // Track which concrete profile 'dynamic' resolves to on THIS decision —
    // a cheap, side-effect-free re-computation purely for reporting; the
    // real decision is made identically inside chooseAction itself.
    if (styleOf[pre.currentSide] === 'dynamic') {
      bump(pre.currentSide, `dynamic->${chooseProfile(pre, pre.currentSide)}`);
    }
...
// ---------------------------------------------------------------------------
// Time-boxed run (user request: "play for one hour... mix up the profiles
// ... decide when and where to change tactics"): keep playing games — round-
// robin across the 6x6 grid of the 5 tactic profiles PLUS "dynamic" (a
// meta-controller that re-picks its profile every single Action based on
// the live game state — see chooseProfile()'s own header comment), with
// roughly 1 in 10 games swapped for a random-vs-random control — until
// DURATION_MS of real wall-clock time has elapsed, then stop and report on
// however many games actually completed.
//
// Fixes a scheduling bug found in the 30-minute round's report: there, the
// pair-rotation index was `gameCount % pairs.length`, and since gameCount
// also drove the "every 10th game is a control game" check, pairings whose
// index happened to land on a multiple-of-10 slot (all 5 "X vs Balanced"
// pairings, since Balanced was first in the profile list) got diverted to
// control roughly twice as often as every other pairing — a real, if minor,
// sample-size imbalance. Fixed here by giving the pair-rotation its own
// cursor that only advances on actual profile games, decoupled from the
// control-game counter entirely.
// ---------------------------------------------------------------------------

const PROFILES: Style[] = ['balanced', 'ap_econ', 'defense', 'combat_group', 'vp_focus', 'dynamic'];
const avg = (arr: number[]) => (arr.length ? arr.reduce((a, b) => a + b, 0) / arr.length : NaN);

type ResultWithBlock = GameResult & { block: 'control' | 'profile' };
const results: ResultWithBlock[] = [];
let seedCounter = 800001;

const DURATION_MS = 60 * 60 * 1000;
const startTime = Date.now();
const deadline = startTime + DURATION_MS;

console.log(`Conflict of Heroes — time-boxed self-play analysis (Mission 1 "Partisans", v3 rules, dynamic tactic-switching)`);
console.log(`Running for ${DURATION_MS / 60000} minutes (until ${new Date(deadline).toISOString()})...\n`);

const pairs: [Style, Style][] = [];
for (const sa of PROFILES) for (const sb of PROFILES) pairs.push([sa, sb]);
...
while (Date.now() < deadline) {
  gameCount++;
  if (gameCount % 10 === 0) {
    results.push({ ...playGame(seedCounter++, 'random', 'random'), block: 'control' });
  } else {
    const [sa, sb] = pairs[pairCursor % pairs.length]!;
    pairCursor++;
    results.push({ ...playGame(seedCounter++, sa, sb), block: 'profile' });
  }
  const now = Date.now();
  if (now - lastProgressLog >= 60_000) {
    const elapsedMin = ((now - startTime) / 60000).toFixed(1);
    console.log(`  ...progress: ${gameCount} games completed after ${elapsedMin} min`);
    lastProgressLog = now;
  }
}
const actualElapsedMin = ((Date.now() - startTime) / 60000).toFixed(2);
console.log(`\nStopped after ${actualElapsedMin} minutes: ${gameCount} games played.\n`);
...
const winsByStyleAsA: Record<Style, number> = { balanced: 0, ap_econ: 0, defense: 0, combat_group: 0, vp_focus: 0, random: 0, dynamic: 0 };
const winsByStyleAsB: Record<Style, number> = { balanced: 0, ap_econ: 0, defense: 0, combat_group: 0, vp_focus: 0, random: 0, dynamic: 0 };
const gamesByStyleAsA: Record<Style, number> = { balanced: 0, ap_econ: 0, defense: 0, combat_group: 0, vp_focus: 0, random: 0, dynamic: 0 };
const gamesByStyleAsB: Record<Style, number> = { balanced: 0, ap_econ: 0, defense: 0, combat_group: 0, vp_focus: 0, random: 0, dynamic: 0 };
let marginSum = 0;
const marginByWinnerStyle: Record<Style, number[]> = { balanced: [], ap_econ: [], defense: [], combat_group: [], vp_focus: [], random: [], dynamic: [] };
const leadRoundByWinnerStyle: Record<Style, number[]> = { balanced: [], ap_econ: [], defense: [], combat_group: [], vp_focus: [], random: [], dynamic: [] };
const coverageTotals: Record<Style, Record<string, number>> = {
  balanced: {}, ap_econ: {}, defense: {}, combat_group: {}, vp_focus: {}, random: {}, dynamic: {},
};
...
console.log(`\n--- Control block (${results.filter((r) => r.block === 'control').length} games, random vs random — rules-exerciser) ---`);
...
console.log(`\n--- Profile block (${results.filter((r) => r.block === 'profile').length} games, 6x6 tactic-profile grid incl. dynamic) ---`);
const profileGames = results.filter((r) => r.block === 'profile');
const profileAWins = profileGames.filter((r) => r.winner === 'A').length;
console.log(`  Side A: ${profileAWins}/${profileGames.length} (${((100 * profileAWins) / profileGames.length).toFixed(1)}%)`);
console.log(`  Average VP margin: ${(profileGames.reduce((n, r) => n + r.margin, 0) / profileGames.length).toFixed(2)}`);
