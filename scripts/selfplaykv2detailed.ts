...
function moveScoreFor(
  ...
  const extras = cover + los + apEff + elev + hill;
...
function playGame(seed: number, styleA: Style, styleB: Style): GameResult {
  ...
  while (state.phase === 'playing' && steps < 10000) {
    ...
    const verifyGroupEconomy = (side: SideId, memberIds: UnitId[], expGroupCost: number, skipCost = false) => {
      if (!skipCost) {
...
console.log(`Conflict of Heroes — 120-game detailed self-play analysis (AtB Firefight 9 - KV2, v3 rules)`);
console.log(`Scheduled 120 games (20 random-vs-random control + 100 across the 5x5 heuristic grid)...\n`);

for (let k = 0; k < 20; k++) results.push({ ...playGame(seedCounter++, 'random', 'random'), block: 'control' });
...
try {
  const outPath = path.join(process.cwd(), 'selfplaykv2detailed-export.json');
