/**
 * 200-game self-play harness — "German doctrine" round, forked from the
 * validated scripts/selfplaykv2detailed.ts (120-game, per-unit-type +
 * hex-hit-heatmap). Same independent-rule-oracle technique as scripts/
 * conformance.ts throughout; same per-unit-type + hit-heatmap tracking as
 * the prior round, PLUS a new per-side MOVE heatmap (German shaded grey,
 * Soviet shaded red) per this round's own request.
 *
 * User's explicit doctrine change for the German AI (side A), applied
 * unconditionally on top of the existing 5-style weight system (it isn't a
 * 6th style — it's a hard rule layered over all five, the same way the
 * existing "Soviets fear the Flak88" target bonus already applies
 * regardless of style):
 *   1. Only the Flak 18 88mm may freely engage the KV2a — its #1 mission.
 *   2. Every other German attacker (mainly the Panzers — infantry's Firepower
 *      against the KV2a's blue DR was already close to hopeless before this
 *      change) may ONLY fire on the KV2a if the shot is a flank shot (Close
 *      Combat always qualifies, §6.11), or if spending up to 2 CAPs on the
 *      dice mod (§3.2) brings the Hit Number to <=10 (a 2d6-vs-HN roll needs
 *      >=10 on 6/36 = 16.7% of rolls — the user's own "17% or greater"
 *      threshold IS that exact rules number). Otherwise it skips the KV2a
 *      entirely and looks for another target — "German units should try to
 *      destroy the other units."
 *   3. Non-Flak88/Truck German Wave-1 units are split into three sectors by
 *      a stable per-unit-id hash — 'road' (advance south hugging the
 *      Mission's own N/S road at column J, entryDescription's "Road
 *      4-J01"), 'southeast', 'southwest' — instead of massing on the same
 *      central columns the 120-game round's own hit-heatmap showed getting
 *      chewed up (J06/H03/P02, all in the entry zone's first few Rows).
 *   4. The Flak 18 88mm (and its Opel tow truck) gets a standing bonus to
 *      close distance to the KV2a as fast as possible, on top of the
 *      existing hill-seeking bonus — "get the 88 Flak to kill the KV2a as
 *      quickly as possible."
 * A light, symmetrical touch on the Soviet side per the same user note
 * ("Soviets want to preserve it"): the KV2a specifically avoids ending a
 * Move within a spotted Flak88's LOS+range, where every other Unit's
 * default move-scoring already pulls it toward the nearest enemy.
 *
 *   npx tsx scripts/selfplaykv2doctrine.ts
 */
import {
  reduce,
  legalActions,
...
/** User's own tactical note: "get the 88 Flak to kill the KV2a as quickly as
 *  possible" — a standing pull toward the KV2a's current Hex, additive with
 *  `flak88HillBonus` above (climb AND close distance), shared by its Opel
 *  tow truck so the two don't get separated advancing at different paces. */
function flak88RushBonus(state: GameState, unit: Unit, toHexId: HexId): number {
  if (unit.templateId !== 'ger-flak18-88mm' && unit.templateId !== 'ger-truck-opel') return 0;
  const kv2 = Object.values(state.units).find((u) => u.templateId === 'sov-kv2a');
  if (!kv2) return 0;
  return -distance(parseHexId(toHexId), parseHexId(kv2.hexId)) * 1.2;
}

/** User's own tactical note: "consider a strategy where some of the German
 *  forces go South using road, or to the Southeast and another set goes to
 *  the Southwest ... divide and conquer" — a direct response to the
 *  120-game round's own hit-heatmap, which found the fighting (and the
 *  casualties) concentrated hard on the same few central-column Hexes right
 *  at the entry zone (J06/H03/P02, all within a few Rows of where every
 *  German Unit enters at column J/q=9 — see entryDescription's own "Road
 *  4-J01"). Deterministic per-Unit-id hash, not per-game random, so a given
 *  Unit keeps the same sector assignment for its whole game rather than
 *  wandering between them Turn to Turn. Only applies to Wave-1 combat
 *  Units — the Flak88/Truck have their own dedicated rush behavior above,
 *  and Reinforcements arriving on their own already have nothing to
 *  "disperse" from. */
const ROAD_COLUMN = 9; // q index of column J — this Mission's own N/S road (entryDescription: "Road 4-J01")
type Sector = 'road' | 'southeast' | 'southwest';
function sectorFor(unitId: UnitId): Sector {
  let h = 0;
  for (let i = 0; i < unitId.length; i++) h = (h * 31 + unitId.charCodeAt(i)) >>> 0;
  const sectors: Sector[] = ['road', 'southeast', 'southwest'];
  return sectors[h % 3]!;
}
function sectorTerm(unit: Unit, toHexId: HexId): number {
  if (unit.side !== 'A' || unit.templateId === 'ger-flak18-88mm' || unit.templateId === 'ger-truck-opel') return 0;
  const { q, r } = parseHexId(toHexId);
  const southward = r * 0.4; // always reward pressing south, toward the objectives
  const lateral = q - ROAD_COLUMN; // negative = west of the road, positive = east
  const sector = sectorFor(unit.id);
  if (sector === 'road') return southward - Math.abs(lateral) * 0.5;
  if (sector === 'southeast') return southward + Math.max(0, lateral) * 0.5;
  return southward + Math.max(0, -lateral) * 0.5; // southwest
}

/** User's own tactical note: "Soviets want to preserve it [the KV2a]" — the
 *  one Soviet-side change this round. Every other Unit's default move score
 *  already pulls it toward the nearest enemy (moveScoreFor's own
 *  `-Math.min(...enemies distance)` term below); this Hex-specific carve-out
 *  keeps the KV2a from walking into a spotted Flak88's own LOS+range, the
 *  one Unit type genuinely built to kill it. */
function kv2aPreservationTerm(state: GameState, unit: Unit, toHexId: HexId, enemies: Unit[]): number {
  if (unit.templateId !== 'sov-kv2a') return 0;
  const flak = enemies.find((e) => e.templateId === 'ger-flak18-88mm');
  if (!flak) return 0;
  const flakRange = state.templates['ger-flak18-88mm']!.range;
  const exposed = distance(parseHexId(toHexId), parseHexId(flak.hexId)) <= flakRange && hasLOS(state, toHexId, flak.hexId);
  return exposed ? -6 : 1;
}
...
function moveScoreFor(
  ...
  const rush = flak88RushBonus(state, unit, toHexId);
  const sector = sectorTerm(unit, toHexId);
  const preserve = kv2aPreservationTerm(state, unit, toHexId, enemies);
  const extras = cover + los + apEff + elev + hill + rush + sector + preserve;
...
function chooseAction(state: GameState, styleOf: Record<SideId, Style>, rand: () => number): Action {
  ...
  // User's own doctrine: "the only thing worth attacking the KV2a is the 88
  // Flak and that should be its number 1 mission" — a bonus large enough to
  // always outweigh a normal Hit-Number-based target choice once the Flak88
  // itself has a legal shot on it.
  const flak88PrioritizesKv2a = (attackerId: UnitId, targetId: UnitId) =>
    state.units[attackerId]?.templateId === 'ger-flak18-88mm' && state.units[targetId]?.templateId === 'sov-kv2a' ? 8 : 0;
  // User's own doctrine: "the tanks shouldn't even try [to attack the KV2a]
  // unless they can get a flank on it or spend CAPs to give it a reasonable
  // chance, say 17% or greater." A 2d6-vs-Hit-Number roll needs >=10 on
  // 6/36 = 16.7% of rolls — the user's own "17%" IS that exact rules number,
  // so "Hit Number <= 10, achievable with up to 2 CAPs (§3.2)" is a literal,
  // not approximate, translation of the request. Close Combat always
  // resolves vs Flank DR (§6.11, `expectCC` has no isFlank field at all) so
  // every CC attack on the KV2a already qualifies as "a flank" by rule.
  const canEngageKv2a = (attackerId: UnitId, targetId: UnitId, isCC: boolean): boolean => {
    const target = state.units[targetId]!;
    if (target.templateId !== 'sov-kv2a') return true; // gate is specific to the KV2a
    const attacker = state.units[attackerId]!;
    if (attacker.side !== 'A' || attacker.templateId === 'ger-flak18-88mm' || isCC) return true;
    const ex = expectFire(state, attacker, target);
    if (!ex) return false;
    if (ex.isFlank) return true;
    const availableReduction = Math.min(2, state.players.A.capCurrent);
    return ex.hitNumber - availableReduction <= 10;
  };
  const capDiceModForKv2 = (attackerId: UnitId, targetId: UnitId, isCC: boolean): number => {
    const target = state.units[targetId]!;
    const attacker = state.units[attackerId]!;
    if (target.templateId !== 'sov-kv2a' || attacker.side !== 'A' || attacker.templateId === 'ger-flak18-88mm' || isCC) return 0;
    const ex = expectFire(state, attacker, target);
    if (!ex || ex.isFlank) return 0;
    const needed = Math.max(0, ex.hitNumber - 10);
    return Math.min(2, needed, state.players.A.capCurrent);
  };
  const ccScore = (a: { attackerId: string; targetId: string }) =>
    -expectCC(state, state.units[a.attackerId]!, state.units[a.targetId]!).hitNumber + fearsFlak88(a.targetId) + flak88PrioritizesKv2a(a.attackerId, a.targetId);
  const fireScore = (a: { attackerId: string; targetId: string }) => {
    ...
    return ex ? -ex.hitNumber + fearsFlak88(a.targetId) + flak88PrioritizesKv2a(a.attackerId, a.targetId) : -Infinity;
  };
   ...
   *  attaching the CAP dice-mod boost (see capDiceModForKv2 above) whenever
   *  the shot is a non-flank Fire on the KV2a that needs it to clear the
   *  doctrine's own 17%/Hit-Number-10 bar. */
  const groupUpgrade = (leaderId: UnitId, targetId: UnitId): Action => {
    ...
    const diceMod = capDiceModForKv2(leaderId, targetId, isCC);
    ...
    if (supporterIds.length === 0) {
      return isCC
        ...
        : { type: 'FIRE', attackerId: leaderId, targetId, ...(diceMod ? { capDiceMod: diceMod } : {}) };
    }
    return { type: 'GROUP_ATTACK', leaderId, supporterIds, targetId, ...(diceMod ? { capDiceMod: diceMod } : {}) };
  };
  ...
  // Combat priority is shared by all four profiles (Close Combat > Fire, both
  ...
  // so there's never a reason for any profile to decline it). User's own
  // doctrine change: on the German side, a target the KV2a-engagement gate
  // (canEngageKv2a, defined above) rejects is dropped from consideration
  // entirely — the AI just looks for the next-best (non-KV2a) target, which
  // is what "German units should try to destroy the other units" falls out
  // to once the KV2a option is off the table. Soviet-side (and the CC list,
  // which the gate always passes per §6.11) are untouched.
  const ccGated = side === 'A' ? cc.filter((a) => canEngageKv2a(a.attackerId, a.targetId, true)) : cc;
  const firesGated = side === 'A' ? fires.filter((a) => canEngageKv2a(a.attackerId, a.targetId, false)) : fires;
  const c = bestBy(ccGated, ccScore);
  if (c) return groupUpgrade(c.attackerId, c.targetId);
  const f = bestBy(firesGated, fireScore);
...
function playGame(seed: number, styleA: Style, styleB: Style): GameResult {
  ...
  while (state.phase === 'playing' && steps < 10000) {
    ...
    const verifyGroupEconomy = (side: SideId, memberIds: UnitId[], expGroupCost: number, skipCost = false, expCapSpend = 0) => {
      if (!skipCost) {
        // §3.2/§10.8: a Group Attack's own capDiceMod (never set by any
        // earlier round's AI, so this branch was never exercised until the
        // German doctrine's KV2a-engagement CAP boost) is real CAP spend on
        // top of the "no AP-cost CAP spend" a fresh Group Action otherwise
        // makes — `expCapSpend` defaults to 0, preserving every other call
        // site's existing behavior exactly.
        check(post.players[side].capCurrent === preCap[side] - expCapSpend, '3.3', `Group action CAP after engine ${post.players[side].capCurrent} != expected ${preCap[side] - expCapSpend} (${aStr})`);
      }
      const sp = parseSpent(res.events);
      if (skipCost) {
...
let seedCounter = 900001;

const HEURISTICS: Style[] = ['balanced', 'ap_econ', 'defense', 'combat_group', 'vp_focus'];
console.log(`Conflict of Heroes — 200-game "German doctrine" self-play analysis (AtB Firefight 9 - KV2, v3 rules)`);
console.log(`Scheduled 200 games (25 random-vs-random control + 175 across the 5x5 heuristic grid, 7 each)...\n`);
console.log(`German (side A) doctrine active in every non-random game: KV2a engagement gated to Flak88/flank/CAP->=17%, Wave-1 units split into road/SE/SW sectors, Flak88+truck rush the KV2a. Soviet KV2a avoids a spotted Flak88's LOS+range.\n`);
...
try {
  const outPath = path.join(process.cwd(), 'selfplaykv2doctrine-export.json');
