/**
 * Time-boxed self-play harness — seventh round, follow-up to
 * scripts/selfplay1000tactics.ts (1000 games, same tactics-sheet-driven
 * profiles). Same independent-rule-oracle technique as scripts/
 * conformance.ts throughout; same 4 Category profiles + balanced + random
 * as the prior round (see that file's header for the full tactics-sheet
 * row-by-row mapping — unchanged here).
 *
 * The only thing this round changes: the user asked to "play yourself for
 * 30 minutes" rather than a fixed game count. The schedule loop below checks
 * real wall-clock time (Date.now()) against a 30-minute deadline instead of
 * counting down a fixed number of games — it keeps playing (round-robin
 * across the same 5x5 profile grid, ~1 game in 10 swapped for a random-vs-
 * random control) until the deadline passes, then stops and reports on
 * however many games actually completed.
 *
 *   npx tsx scripts/selfplay30min.ts
 */
import {
  reduce,
  legalActions,
...
// ---------------------------------------------------------------------------
// Time-boxed run (user request: "play yourself for 30 minutes"), not a fixed
// game count: keep playing games — round-robin across the same 5x5 grid of
// tactic-profile pairings as the last report, with roughly 1 in 10 games
// swapped for a random-vs-random control — until DURATION_MS of real wall-
// clock time has elapsed since the process started, then stop and report on
// however many games actually completed. Individual games are fast (a few
// hundred Actions each), so this is expected to complete several thousand
// games in 30 minutes — a strict superset of the last round's 1,000-game
// sample, same profiles, same Mission.
// ---------------------------------------------------------------------------

const PROFILES: Style[] = ['balanced', 'ap_econ', 'defense', 'combat_group', 'vp_focus'];
...
let seedCounter = 700001;

const DURATION_MS = 30 * 60 * 1000;
const startTime = Date.now();
const deadline = startTime + DURATION_MS;

console.log(`Conflict of Heroes — time-boxed self-play analysis (Mission 1 "Partisans", v3 rules, tactics-sheet-driven profiles)`);
...
while (Date.now() < deadline) {
  ...
  if (gameCount % 10 === 0) {
    ...
    const [sa, sb] = pairs[gameCount % pairs.length]!;
...
let marginSum = 0;
const marginByWinnerStyle: Record<Style, number[]> = { balanced: [], ap_econ: [], defense: [], combat_group: [], vp_focus: [], random: [] };
const leadRoundByWinnerStyle: Record<Style, number[]> = { balanced: [], ap_econ: [], defense: [], combat_group: [], vp_focus: [], random: [] };
const coverageTotals: Record<Style, Record<string, number>> = {
  balanced: {}, ap_econ: {}, defense: {}, combat_group: {}, vp_focus: {}, random: {},
};
...
console.log(`\n--- Profile block (${results.filter((r) => r.block === 'profile').length} games, 5x5 tactic-profile grid) ---`);
