/**
 * 1000-game self-play analysis harness — sixth round, follow-up to
 * scripts/selfplay.ts (25), scripts/selfplay500.ts (500, Group Actions),
 * scripts/selfplay1000.ts (1000, win-weighted styles),
 * scripts/selfplay1000cover.ts (1000, cover-seeking), and
 * scripts/selfplay1000rl.ts (1000, Hasty Defense + LOS-spread + adaptive
 * weight-search). Same independent-rule-oracle technique as scripts/
 * conformance.ts throughout.
 *
 * This round is driven directly by the user-curated tactics reference sheet
 * at reference/conflict_of_heroes_tactics.xlsx (37 rows, read fresh for this
 * round — the user added row 3 "Stress Cap Avoidance" and re-categorized
 * several rows into "Combat & Group Actions" after the sheet was first
 * built). Four styles below, one per sheet Category, each weighting its
 * movement/action scoring toward that Category's tactics (see PROFILE_
 * WEIGHTS' own header comment for the full row-by-row mapping) — plus
 * "balanced" as a neutral no-emphasis baseline and "random" as the control/
 * rules-exerciser, unchanged in kind from every prior round. The goal (the
 * user's own framing): find out which tactics/categories actually correlate
 * with more VP and more wins, for which side, against which opponent —
 * i.e. empirically test the sheet's own "Impact Rating" column, which was
 * explicitly flagged as an unproven starting guess, not a settled fact.
 *
 *   npx tsx scripts/selfplay1000tactics.ts
 */
import {
  reduce,
  legalActions,
...
// ---------------------------------------------------------------------------
// Build the 1000-game schedule: 100 random-vs-random (control/rules-
// exerciser) + 900 across the 5x5 grid of the 4 tactic-profile styles plus
// "balanced" (36 games per pairing) — every profile plays every profile
// (including itself) as both Germans and Soviets, which is exactly what's
// needed to answer "which tactics work better for which side and in which
// condition [opponent]."
// ---------------------------------------------------------------------------
...
let seedCounter = 600001;

console.log(`Conflict of Heroes — 1000-game self-play analysis (Mission 1 "Partisans", v3 rules, tactics-sheet-driven profiles)`);
console.log(`Scheduled 1000 games (100 random-vs-random control + 900 across the 5x5 tactic-profile grid)...\n`);
...
// Profile block (900 games): every one of the 25 profile pairings, 36 seeds each.
const pairs: [Style, Style][] = [];
for (const sa of PROFILES) for (const sb of PROFILES) pairs.push([sa, sb]);
for (const [sa, sb] of pairs) {
  for (let k = 0; k < 36; k++) results.push({ ...playGame(seedCounter++, sa, sb), block: 'profile' });
}
...
console.log(`\n--- Profile block (900 games, 5x5 tactic-profile grid) ---`);
