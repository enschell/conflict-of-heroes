/**
 * 500-game self-play analysis harness (ad-hoc, user-requested follow-up to
 * scripts/selfplay.ts). Same independent-rule-oracle technique as
 * scripts/conformance.ts / scripts/selfplay.ts, extended to:
 *
 *   - Group Actions (§10): reinforcement waves enter as one batched Group
 *     Entry when possible (mirrors the live UI's "Enter now"); a Fire/Close
 *     Combat leader groups in any qualifying adjacent Fresh supporters
 *     (§10.6-10.8); a cluster of Fresh units moving together uses one Group
 *     Move (§10.2-10.4); a cluster of Hit units rallies together (§10.9).
 *   - A fifth "random" strategy that samples uniformly from every legal
 *     Action (including PIVOT/STALL, which the four heuristic styles never
 *     choose) plus the constructed Group-Action candidates, per the user's
 *     "play with random moves for some time" instruction.
 *   - A much larger schedule (25 style pairings x 20 seeds = 500 games) so
 *     both random exploration and heuristic "try to win" play get exercised.
 *
 *   npx tsx scripts/selfplay500.ts
 */
import {
  reduce,
  legalActions,
...
// ---------------------------------------------------------------------------
// Build the 500-game schedule: all 25 ordered style pairings (5x5), 20 seeds
// each = 500 games. This covers every heuristic matchup plus "random" mixed
// in against everything (9 of the 25 pairings involve random).
// ---------------------------------------------------------------------------

const games: { seed: number; styleA: Style; styleB: Style }[] = [];
let seedCounter = 90001;
for (const sa of STYLES) {
  for (const sb of STYLES) {
    for (let k = 0; k < 20; k++) games.push({ seed: seedCounter++, styleA: sa, styleB: sb });
  }
}

console.log(`Conflict of Heroes — 500-game self-play analysis (Mission 1 "Partisans", v3 rules, Group Actions)`);
console.log(`Scheduled ${games.length} games...\n`);

const results: GameResult[] = games.map((g) => playGame(g.seed, g.styleA, g.styleB));

// ---------------------------------------------------------------------------
// Report (aggregate only — 500 rows is too much for a console; violations
