/**
 * 1000-game self-play analysis harness — fourth round, follow-up to
 * scripts/selfplay.ts (25), scripts/selfplay500.ts (500, added Group
 * Actions), and scripts/selfplay1000.ts (1000, win-weighted strategy
 * selection). Same independent-rule-oracle technique as scripts/
 * conformance.ts throughout. New in this round, per the user's explicit ask:
 *
 *   - Cover-seeking movement: every heuristic style's move scoring now adds
 *     a `coverTerm` (the destination Hex's §6.4 Defense Modifier, minus a
 *     penalty for leaving BETTER cover than the destination offers) on top
 *     of its existing tactical driver — "consider using or staying in cover
 *     ... to increase chance of not getting hit." Objective still primarily
 *     beelines for the VP Hex (the user's own acknowledged trade-off — "if
 *     the strategy is to take the objective this may have to be sacrificed")
 *     but gets a much smaller cover weight as a tie-breaker among otherwise
 *     equal approaches.
 *   - Schedule changed to the user's explicit split: exactly 100 games are
 *     random-vs-random (the control/rules-exerciser group, unchanged from
 *     prior rounds); the other 900 are heuristic-vs-heuristic only — no
 *     other pairing involves "random" this round.
 *   - Reinforcements: unchanged from prior rounds (already always entered,
 *     batched into one Group Entry, the instant they're eligible) — kept
 *     exactly as-is per the user's "make use of reinforcements" reminder.
 *
 *   npx tsx scripts/selfplay1000cover.ts
 */
import {
  reduce,
  legalActions,
...
function moveScoreFor(style: Style, state: GameState, fromHexId: HexId, toHexId: HexId, enemies: Unit[]): number {
  // §6.4: Objective explicitly may sacrifice cover to reach the VP Hex faster
  // (the user's own caveat) — cover only breaks a tie among otherwise-equal
  // approach distances, at a small fraction of its normal weight.
  if (style === 'objective') {
    const dist = -distance(parseHexId(toHexId), parseHexId(OBJECTIVE_HEX));
    return dist + coverTerm(state, fromHexId, toHexId) * 0.15;
  }
  const cover = coverTerm(state, fromHexId, toHexId);
  if (!enemies.length) return cover;
  if (style === 'marksman') {
    const d = Math.min(...enemies.map((e) => distance(parseHexId(toHexId), parseHexId(e.hexId))));
    return (d <= 1 ? -100 - d : -d) + cover;
  }
  return -Math.min(...enemies.map((e) => distance(parseHexId(toHexId), parseHexId(e.hexId)))) + cover;
}
  ...
  if (moves.length) {
    ...
    if (moveCluster.length >= 2) {
      const memberMoves = moveCluster.map((u) => {
        ...
        const best =
          style === 'assault'
            ? bestBy(candidates, (a) => (enemies.some((e) => e.hexId === a.toHexId) ? 1000 : moveScoreFor(style, state, u.hexId, a.toHexId, enemies)))!
            : bestBy(candidates, (a) => moveScoreFor(style, state, u.hexId, a.toHexId, enemies))!;
    ...
    return bestBy(moves, (a) => moveScoreFor(style, state, state.units[a.unitId]!.hexId, a.toHexId, enemies))!;
  }
...
// ---------------------------------------------------------------------------
// Build the 1000-game schedule per the user's explicit split: exactly 100
// random-vs-random (control/rules-exerciser, unchanged in kind from prior
// rounds) + 900 heuristic-vs-heuristic (no other pairing touches "random"
// this round), spread as evenly as possible over the 4x4=16 style pairings
// via round-robin so the cover-seeking change gets uniform, comparable
// coverage across every matchup rather than reusing a possibly-stale weight
// scheme from the last report.
// ---------------------------------------------------------------------------

const HEURISTICS: Style[] = ['balanced', 'assault', 'marksman', 'objective'];

const games: { seed: number; styleA: Style; styleB: Style; block: 'control' | 'heuristic' }[] = [];
let seedCounter = 400001;

// Control block (100 games): random vs random only.
for (let k = 0; k < 100; k++) games.push({ seed: seedCounter++, styleA: 'random', styleB: 'random', block: 'control' });

// Heuristic block (900 games): every one of the 16 style pairings, round-robin.
...
for (let i = 0; i < 900; i++) {
  ...
  games.push({ seed: seedCounter++, styleA: sa, styleB: sb, block: 'heuristic' });
}

console.log(`Conflict of Heroes — 1000-game self-play analysis (Mission 1 "Partisans", v3 rules, cover-seeking movement)`);
console.log(`Scheduled ${games.length} games (100 random-vs-random control + 900 heuristic-vs-heuristic)...\n`);
...
console.log(`\n--- Heuristic block (900 games, all 16 style pairings, cover-seeking movement) ---`);
